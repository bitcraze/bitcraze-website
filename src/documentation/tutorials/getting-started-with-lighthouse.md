...
* The latest version of the [Crazyflie client](https://github.com/bitcraze/crazyflie-clients-python/releases). Check out the [Installation instructions](/documentation/repository/crazyflie-clients-python/master/installation/install/).
{% endsi_step %}
...
For mounting the {% poplink lighthouse-deck %} on a Crazyflie check out the {% id_link getting-started-expansion-decks %} tutorial. Be careful When mounting the Lighthouse positioning deck using the long pins. Make sure they do not block the laser sweeps from the base stations, as this could affect the drone's performance.
{% endsi_step %}
...
Please follow the [firmware upgrade instructions](/documentation/repository/crazyflie-clients-python/master/userguides/userguide_client/#firmware-upgrade).
...
4. Put the wanted channel (1 to 4) in **Change Channel** and click the **Set Channel** button. Each basestation should have a unique channel.
...
* Remove the mirrors or big large reflective items in the area.
...
This section explains how to obtain the calibration and geometry data and configure your Crazyflie for flight using the Lighthouse positioning system.
...
Click the **Change system type** button and select the appropriate type. The system type is stored in the Crazyflie and is automatically used the next time it starts.
{% endsi_step %}

{% si_step Wait for calibration of the base stations %}
Place the Crazyflie within the flight area so that it has a clear line of sight to both base stations. Wait approximately 20 seconds for the calibration data to be received before proceeding.
...
The goal is to generate a configuration file that is stored in the Crazyflie and used to estimate its position.
...
* Place it 1 m along the positive X-axis and press **Start measurement**
* Place it in multiple positions in the XY-plane (not on the X-axis) and press **Start measurement**
* Carry the Crazyflie to several positions in your flight space and record samples by quickly rotating it left-right first and then holding it still.
...
The Lighthouse Deck’s LED should now be green, and the visualization should show the base stations with their channels and the Crazyflie as a blue dot.
...
* Check out the [cfclient lighthouse tab user guide](/documentation/repository/crazyflie-clients-python/master/userguides/userguide_client/lighthouse_tab/) for a detailed explanation of each button.
